...
let distroToNatural : LinuxDistro → Natural =
      λ(d : LinuxDistro) →
        merge
          { Debian = 1
          , Ubuntu = 2
...
let distroToText : LinuxDistro → Text =
      λ(d : LinuxDistro) →
        merge
          { Debian = "Debian"
          , Ubuntu = "Ubuntu"
...
let platformToText : Platform → Text =
      λ(p : Platform) →
        merge
...
let platformToNatural : Platform → Natural =
      λ(p : Platform) →
        merge
...
let Architecture =
      ...
      | A_ARM64
      >
let archToText : Architecture → Text =
      λ(a : Architecture) →
        merge
          { A_64 = "x86_64"
          , A_32 = "i386"
...
let MakeSpec =
      { makeEnv : Optional EnvSpec
      , makeArgs : List Text
      }
let InstallFileRule =
      < InstallFileRule :
          { installSource : Text
          , installDest : Optional Text
          }
      | InstallFilePatternRule :
          { installPattern : List Text
          }
      >
let SymlinkSpec =
      { target : Text
...
let ToolRequirements = Map Tool (Map Version (Map PlatformText (Map Text Requirements)))
...
let Bindist = { bindist : Text, subdir : Text, hash : Text, platforms : List { platform : Platform, verRange : List Text } }
...
let ghcBaseVi =
      ...
      λ(archSpec : ArchitectureValueSpec) →
        VersionInfo::{ viTags = tags
          , viChangeLog = Some
              "${baseUrl}/docs/html/users_guide/release-7-10-1.html"
          , viSourceDL = Some
              (DownloadInfo::{ dlUri = "${baseUrl}/ghc-${v}-src.tar.xz"
                  , dlSubdir = Some (TarDir.RealDir "ghc-${v}")
                  , dlHash = "${srcHash}"
                  }
              )
          , viTestDL = Some
              (DownloadInfo::{ dlUri = "${baseUrl}/ghc-${v}-testsuite.tar.xz"
...
let toKeyValueMap =
      ...
      → Prelude.List.map
          ...
          (λ(m : { mapKey : k, mapValue : v }) → { mapKey = (f m.mapKey), mapValue = m.mapValue })
          list
...
let mergePlatformSpec =
      ...
      → Prelude.List.fold
          ...
          platforms
          PlatformSpec
          (λ(a : { platform : Platform, verRange : List Text }) → λ(b : PlatformSpec) →
            let newPlat =
                       { mapKey = a.platform
                       , mapValue = Prelude.List.map
                                      Text
                                      { mapKey : VersionRange, mapValue : DownloadInfo.Type }
                                      (λ(vr : Text) →
                                        { mapKey = vr
                                        , mapValue =  DownloadInfo::{ dlUri = "${bindist}"
            ...
            in if Prelude.List.any
                    ...
                    (λ(l : { mapKey : Platform, mapValue : PlatformVersionSpec }) → Prelude.Natural.equal (platformToNatural l.mapKey) (platformToNatural newPlat.mapKey))
                    b
               then Prelude.List.map
                      { mapKey : Platform, mapValue : PlatformVersionSpec }
                      { mapKey : Platform, mapValue : PlatformVersionSpec }
                      (λ(l : { mapKey : Platform, mapValue : PlatformVersionSpec }) →
                        if Prelude.Natural.equal (platformToNatural l.mapKey) (platformToNatural newPlat.mapKey)
                        then { mapKey = l.mapKey, mapValue = l.mapValue # newPlat.mapValue }
                        else l
                      )
                      b
               else [ newPlat ] # b)
...
let mergeArchitectureSpec =
      ...
      → let archSpec = Prelude.List.fold
              ArchBindist
              archBindists
              (List { mapKey : Architecture, mapValue : PlatformSpec })
              (λ(a : ArchBindist) → λ(p : (List { mapKey : Architecture, mapValue : PlatformSpec })) →
                 let platformSpec =
                       Prelude.List.fold
                         Bindist
                         a.bindists
                         PlatformSpec
                         (λ(b : Bindist) → λ(acc : PlatformSpec) →
                           mergePlatformSpec b.platforms b.bindist b.subdir b.hash acc
                         )
                         ([] : PlatformSpec)
                 in [ { mapKey = a.arch
                      , mapValue = platformSpec
                      } ] # p)
              ([] : (List { mapKey : Architecture, mapValue : PlatformSpec }))
        in toKeyValueMap Architecture PlatformValueSpec archToText
             (Prelude.Map.map
                Architecture
                PlatformSpec
                PlatformValueSpec
