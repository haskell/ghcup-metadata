...
-- all ghc versions
let ghc_versions : List { versionInfo : GHCup.VersionInfo.Type, ver : Text } =
      [ { versionInfo = ghc7_10_3, ver = "7.10.3" }
      ]

let ghcs : GHCup.ToolVersionSpec =
      Prelude.List.fold
        { versionInfo : GHCup.VersionInfo.Type, ver : Text }
        ghc_versions
        GHCup.ToolVersionSpec
        (λ(a : { versionInfo : GHCup.VersionInfo.Type, ver : Text }) → λ(b : GHCup.ToolVersionSpec) →
          -- insert the ghc version into the map
          [ Prelude.Map.keyValue GHCup.VersionInfo.Type "${a.ver}" a.versionInfo ] # b
        )
        ([] : GHCup.ToolVersionSpec)
